"""Tailored Anschreiben generation.
...
from jobfit.anschreiben.employer_address import enrich_letter_employer_address
...
from jobfit.anschreiben.generator.render import _render_html, _render_pdf
from jobfit.roles import ROLES

_GENERATED_DIR_NAME = "anschreiben"
PROMPT_DOC_LABEL = "Anschreiben"
PROMPT_MODEL_VAR = "CV_MODEL"
PROMPT_COMMAND_PREFIX = "CV"


def output_path(refnr: str, role_slug: str) -> Path:
    """Return path where generated Anschreiben PDF is saved."""
    out_dir = role_output_dir(role_slug) / _GENERATED_DIR_NAME
    out_dir.mkdir(parents=True, exist_ok=True)
    safe_refnr = re.sub(r"[^\w\-]", "_", refnr)
    return out_dir / f"anschreiben_{safe_refnr}.pdf"


def output_json_path(refnr: str, role_slug: str) -> Path:
    """Return path where generated Anschreiben JSON (LLM output) is saved."""
    return output_path(refnr, role_slug).with_suffix(".json")


def generate(refnr: str, role_slug: str, api_key: str | None = None) -> bytes:
    """Generate a tailored Anschreiben PDF for the given job refnr.

    Returns PDF as bytes. Also saves to data/{role}/output/anschreiben/.
    ...
    if len(job_ctx["beschreibung"]) < 100:
        raise ValueError(
            ...
            "Cannot generate a meaningful Anschreiben."
        )
    ...
    candidate_context = _load_candidate_context(role_slug, refnr)
    if candidate_context:
        logger.info("Candidate context loaded (anschreiben_profile.md / notes file).")

    prompt = _build_prompt(
        job_ctx,
        anonymize_for_llm(cv_text),
        cv_profile,
        matched,
        missing,
        language,
    ...
    logger.info(f"Calling LLM ({provider}/{model}) to generate Anschreiben...")
    letter_data = _call_llm(prompt, api_key)
    letter_data["_model"] = model
    enrich_letter_employer_address(letter_data, job_ctx)
    _restore_identity(letter_data, cv_text, role_slug)

    logger.info("Rendering PDF...")
    pdf_bytes = _render_pdf(letter_data)

    out = output_path(refnr, role_slug)
    out.write_bytes(pdf_bytes)
    output_json_path(refnr, role_slug).write_text(
        json.dumps(letter_data, ensure_ascii=False, indent=2), encoding="utf-8"
    )
...
def generate_html(refnr: str, role_slug: str, api_key: str | None = None) -> str:
    """Generate tailored Anschreiben as HTML string (for preview/debugging)."""
    api_key = resolve_key(api_key, command_prefix=_CV_COMMAND_PREFIX)
    ...
    enrich_letter_employer_address(letter_data, job_ctx)
    _restore_identity(letter_data, cv_text, role_slug)
    return _render_html(letter_data, refnr=refnr, role=role_slug)
...
def preview(refnr: str, role_slug: str, output: str | None = None) -> Path:
    ...
    try:
        ...
        pass
    html = _render_html(letter_data, refnr=refnr, role=role_slug)
    out_path = (
